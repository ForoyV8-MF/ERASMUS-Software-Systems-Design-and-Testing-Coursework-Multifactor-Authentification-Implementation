...
public class SMSCodeTest
{
    ...
    /**
     * Test the isCorrectSMS() method (3 Assertions).
     */
    @Test public void testIsCorrectNumber()
    {
        String number_1 = "+33624242424";
        String number_2 = "445555555555";
        String number_3 = "+445555555555";

        try { assertTrue("Correct number has been rejected", sms_Method.isCorrectNumber(number_1)); }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }

        try { assertTrue("Incorrect number has been accepted", !sms_Method.isCorrectNumber(number_2)); }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }

        try { assertTrue("Correct number has been rejected", sms_Method.isCorrectNumber(number_3)); }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }
    }
    ...
    @Test public void testIsCorrectCode()
    {
        ...
        try { assertTrue("Correct Code has been rejected", sms_Method.isCorrectCode(code_1)); }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }

        try { assertTrue("Incorrect Code has been accepted", !sms_Method.isCorrectCode(code_2)); }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }
    }
    ...
    {
        String number_1 = "33624252627\n";
        String number_2 = "+3324252627\n";

        simulatedOutput = number_1 + number_2;
        System.setIn(new ByteArrayInputStream(simulatedOutput.getBytes()));

        sms_Method.scanner = new Scanner(System.in);

        try
        {
            sms_Method.getInformation();
            assertFalse("Incorrect Number has been accepted", sms_Method.isCorrectNumber(number_1));
        }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }
        ...
        try
        {
            assertTrue("Correct Number has been rejected", sms_Method.isCorrectNumber(number_2));
        }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }
    }
    ...
    @Test public void testVerifyInformation()
    {
        ...
        try
        {
            sms_Method.verifyInformation();

            int code_1 = sms_Method.get_code_sent() - 1;
            simulatedOutput = "" + code_1;

            System.setIn(new ByteArrayInputStream((simulatedOutput + "\n").getBytes()));

            assertFalse("Incorrect Code has been accepted", sms_Method.isCorrectCode(code_1));
        }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }
        ...
        try
        {
            int code_2 = sms_Method.get_code_sent();
            simulatedOutput = "" + code_2;

            System.setIn(new ByteArrayInputStream((simulatedOutput + "\n").getBytes()));

            assertTrue("Correct Code has been rejected", sms_Method.isCorrectCode(code_2));
        }
        catch (Throwable e)
        { collector.addError(e); e.printStackTrace(); }
    }
